...
const disponibles: Producto[] = productos.filter(
...
const bebidas: Producto[] = productos.filter(
    (elemento) => elemento.categoria == "bebidas"
);
console.log("1b. Bebidas:", bebidas.map(p => p.nombre))

// 1c. Filtra los productos que cuestan menos de 2 dólares Y están disponibles
const economicos: Producto[] = productos.filter(
    (elemento) => elemento.precio < 2 && elemento.disponible == true
);
...
const nombres: string[] = productos.map(
...
const resumen = productos.map(
    (elemento) => ({id: elemento.id, nombre: elemento.nombre, precio: elemento.precio})
);
...
const preciosNuevos = productos.map(
    (elemento) => parseFloat((elemento.precio * 1.10).toFixed(2))
);
...
const noExiste = productos.find (
    (elemento) => elemento.id == "p99"
);
if (noExiste) {
...
const tienePopular: boolean = productos[0].tags.includes("popular");
...
const naturales: string[] = productos.filter(
    (elemento) => elemento.tags.includes("natural")).map((elemento) => elemento.nombre
);
...
const bebidasDisponibles: string[] = productos.filter(
    (elemento) => elemento.categoria == "bebidas" && elemento.disponible == true).map((elemento) => elemento.nombre
);
...
const masCaro: Producto | undefined = productos.filter(
    (elemento) => elemento.disponible == true).reduce(
        (prev, current) => (prev.precio > current.precio) ? prev : current, productos[0]
);
...
//     Devuelve boolean
const haySnackPopular: boolean = productos.filter(
    (elemento) => elemento.categoria == "snacks" && elemento.disponible == true).some(
        (elemento) => elemento.tags.includes("popular")
)
