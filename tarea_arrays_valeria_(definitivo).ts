...
const nombres: string[] = productos.map(prod => prod.nombre)
console.log("2a. Nombres:", nombres)

// 2b. Crea un array de objetos con solo { id, nombre, precio }
//     (sin categoría, tags ni disponible)
const resumen = productos.map(prod => ({ id: prod.id, nombre: prod.nombre, precio: prod.precio }))
...
const preciosNuevos = productos.map(prod => Number((prod.precio * 1.1).toFixed(2)))
...
const noExiste = productos.find(prod => prod.id === "p99")
if (noExiste) {
...
const tienePopular: boolean = productos.find(prod => prod.id === "p1")?.tags.includes("popular") ?? false
...
const naturales: string[] = productos.filter(prod => prod.tags.includes("natural")).map(prod => prod.nombre)
...
const bebidasDisponibles: string[] = productos.filter(prod => prod.disponible && prod.categoria === "bebidas").map(prod => prod.nombre)
...
const masCaro: Producto | undefined = productos.filter(prod => prod.disponible).sort((prodA, prodB) => prodB.precio - prodA.precio)[0]
...
//     Devuelve boolean
const haySnackPopular: boolean = productos.filter(prod => prod.disponible && prod.categoria === "snacks").some(prod => prod.tags.includes("popular"))
