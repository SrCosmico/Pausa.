...
const disponibles: Producto[] = productos.filter(p => p.disponible === true)
...
const economicos: Producto[] = productos.filter(p => p.precio < 2 && p.disponible === true)
...
const preciosNuevos = productos.map(p => {
  const nuevoPrecio = p.precio * 1.1
...
// Usamos el truco pro del profesor (?.) y || false por si no lo encuentra
...
const naturales: string[] = productos.filter(p => p.tags.includes("natural")).map(p => p.nombre)
...
const bebidasDisponibles: string[] = productos
  .filter(p => p.disponible === true && p.categoria === "bebidas")
  .map(p => p.nombre)
...
const masCaro: Producto | undefined = productos
  ...
  .reduce((maximo, actual) => {
    if (actual.precio > maximo.precio) {
      return actual
    }
    return maximo
  })
...
const haySnackPopular: boolean = productos
  .filter(p => p.disponible === true && p.categoria === "snacks")
  .some(p => p.tags.includes("popular"))
